...
export default function Sidebar() {
  ...
  useEffect(() => {
    const readMonitoringStatus = () => {
      try {
        const raw = localStorage.getItem(MONITORING_STATUS_KEY);
        if (!raw) {
    ...
    window.addEventListener('storage', readMonitoringStatus);
    window.addEventListener('monitoring-status-changed', readMonitoringStatus);

    return () => {
      window.removeEventListener('storage', readMonitoringStatus);
